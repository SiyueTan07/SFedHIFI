...
class conv_basis(nn.Module):
    ...
    def forward(self, x):
        if self.group == 1: # 1 indicates no grouping, performing convolution on the entire set of convolutional kernels
    ...
    def __init__(self, inplanes, planes, n_basis, basis_size, block_expansion=0, stride=1, downsample=False,
        ...
        if downsample:
            self.downsample = nn.Sequential(
                ...
                layer.BatchNorm2d(planes * block_expansion),
            )
        else:
